...
2. **The crowbar — how do you get it, and the tool-slot tension.** — ✅ **RULED 2026-07-18.** The operator ratified the tool-slot competition as the *intended feature*, verbatim: *"the tension is good. you have to choose — better crack with the propane torch, or easier break-ins."* So the crowbar shares the `tool` slot with the propane torch **on purpose** — it's a build-your-playstyle fork: **the torch feeds the cook/heat minigame (better crack), the crowbar feeds break-ins. Cook or burglar, not both at once.** Acquisition: a durable tool bought from Pete or found (unlike the consumable hitter). **The swap must be legible** — equipping one over the other lands a flat, cursed line so the player *feels* the tradeoff (*"you can cook or you can pry. not both."*), and it must be reversible (re-acquire/re-equip), so the tension is per-outing, not a per-save lockout.
