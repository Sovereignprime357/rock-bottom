...
| **5.5** | **Break-ins, generalized** (barren-quarter buildings, tool/cred-gated) | ⚙️ **NEXT — spec ready** (`SPEC-v22-breakins.md`) | The big one. **5.1 built the heist engine; 5.5 generalizes it** with `tool`/`cred` gate kinds and a non-copper loot table. Inherited a proven engine instead of inventing one. |
| **5.6** | **Fill the barren quarter** | ⛔ BLOCKED by 5.5 | ~25% of the map is empty. It stays empty until 5.5 gives a *reason* to go there (break-in targets). Content without a reason to visit is just more walking. Density, not distance. |
