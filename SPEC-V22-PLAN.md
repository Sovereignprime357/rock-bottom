...
| **4.3** | **Sprite shading / more definition** | ✅ **SHIPPED 2026-08-22** — as the v23 tenebrist palette regrade (branch `v23-grime-cinema`, `SPEC-v23-grime-cinema.md`): 49 palettes re-ranked through one documented S-curve (mauve-shifted shadows, protected lights), same 8 slots/identities, sprite-gate hash re-ratified once with the full ledger in `artifacts/v23/palette-audit.json`. Operator eye gate open. | Operator's "more shading, more definition... just run the sprites." Shipped as a palette re-rank (value curve) rather than per-pixel grid edits — denser ramps inside the same slots, audited, red-tested both directions. |
