...
| **5.3** | **The robbery** (gang areas take your stuff) | ✅ **BUILT 2026-07-18, pending operator review** — branch `v22-robbery`, contract `SPEC-v22-robbery.md` graduated to SPEC.md; `robbery-gate` permanent, suite 17/17. (Note: the spec commit calls this "Wave 5.4" while this table says 5.3 — the hitter shipped out-of-band as "Wave 5.3" and the labels collided. Same wave either way; the table is not renumbered here.) | The economy's first real **sink** — everything so far only adds. Bounded in `§PACKET 2`. Medium size, no new tech (combat + inventory + sprite-visible loss already exist). |
