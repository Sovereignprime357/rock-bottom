...
| **4.3** | **Sprite shading / more definition** | 🔨 **BUILT 2026-07-18 — awaiting operator's eye** (branch `v22-sprite-shading`, contract `SPEC-v22-sprite-shading.md`, pushed without merge) | Roster reshaded denser inside the 32×32 ceiling: shared humanoid `shade`/`worn` ramp roles (46 bases at once), ~20 signature passes, player composite + gear coats, specials incl. brutus and the horse. 94/377 structure untouched; anchors pinned; palette snapshot re-ratified ONCE (two duplicate-cream slots → real ramp steps: larry `#5a2020`, price_guy `#343434`). Suite 18/18, gate red-tested on the real path. **Acceptance = the operator's eye on the atlas — green means machinery, not art.** Weak list named in BRAIN. |
