...
## v21 Wave 4.1 — THE HONEST MAP (2026-07-16)

Implements `SPEC-v21-honest-map.md` under OD-11. This landing changes the physical truth of existing structures and nothing else: no new building, zone, prop, NPC, interaction, route stop, venue, or campaign anchor.

1. **I-DECLARED / I-FLAT-IS-A-CHOICE.** Every record in `BUILDINGS` and `LANDMARK_FACADES` owns exactly one explicit declaration: `solid:true`, or `flat:<registered reason id>`. No false/default value has meaning. A registered flat reason has non-empty explanatory prose. The live reachability audit reaches the center and all four exterior sides of every facade and finds no road, path, rail, existing-building, actor, smoke-spot, or mandatory-anchor overlap; therefore this corpus classifies 28/28 facades as architecture and zero as scenery. `flat` remains a gated schema choice, not a quota.
2. **I-DOOR-PARITY.** The 21 warehouse, industrial, and rowhouse facades that visibly render a south entrance declare the existing centered 60px `doorGap` convention. Their cache paints a centered entrance at the same coordinate; generated off-center bays may remain visibly closed. The seven storefront facades render no explicit door and declare no gap.
3. **I-ONE-COLLISION-SOURCE.** One initialized `STRUCTURES` set merges the two authored tables. One internal solid-structure scanner is the only collision iteration. Player movement, projectiles, load placement, NPCs, and cops call that authority; no collision consumer loops either source table or creates a facade-only second pass.
4. **I-LOAD-EJECTION.** Loading a clear save preserves its coordinates exactly. Loading inside a solid chooses the nearest clear cardinal boundary candidate, world-clamped, with a stable tie order and a bounded derived-ring fallback. Every newly-solid facade receives a real save/load fixture and a repeat-load determinism check.
5. **I-ACTOR-TRAVERSAL.** The contract premise that NPCs/cops already shared `update.js:723` collision was false: that loop belonged to projectiles, while `npc_ai.js` moved actors through all buildings. Wave 4.1 extends the shared authority to actor motion. Obstructed ordinary NPCs/cops retain a stable two-corner detour until clear; both shipped lunge families (charger and fallen-priest dasher) stop both axes on wall impact and enter their existing cooldown rather than curving or sliding. Eight authored actors already begin intentionally embedded in legacy `BUILDINGS` (Tony, Pete, Father O'Malley, his son, Barb, the laundromat lady, Karaoke Mike, and Skid Sherri); each source actor record declares its exact legacy building index, retains that anchor, may leave that one footprint, and joins shared collision once clear. The same identity in another old building receives no exemption. No compatibility exemption applies to a newly-solid facade: a dynamic actor/cop spawned there ejects before moving. Gate smoke requires both far-side arrival and zero solid overlap on every frame, for every newly-solid facade, 56 production charge impacts, and exact-footprint preservation of the eight legacy anchors.
