...
## 2026-07-16 — corpus gate + the retraction (chat Claude in the orchestrator seat, main)

What: merged v20-regular -> main (ccdbf75, 6/6 green). Authored tools/corpus-gate.mjs and wired it FIRST in run-gates.mjs (7/7). MIT LICENSE added, operator name removed from both READMEs, repo public. RESEARCH-ABSURDITY.md + PRIVACY-SWEEP.md landed. Retracted a false section of SPEC-refactor-modularize.md. Specced Landing 3 (SPEC-v20-concessions.md) and delegated it to Fable on branch v20-concessions.
Why: the operator handed over the orchestrator seat for the day. First decision made from it was a gate, not a feature.
Decided: corpus-gate runs before every other gate. The six existing gates all check the GAME; nothing checked the CORPUS — VIBE/SPEC/AGENTS/BRAIN — which is what every future version is reproduced from. A suite that protects the output but not the seed protects the wrong thing. Two checks: HEAD-vs-view line-count sanity (catches a stale mount) and staged-deletion threshold. Escape hatch CORPUS_GATE_ALLOW exists on purpose — bypassable, never silently.
Decided: the lanyard is flavor, not state (Landing 3). Packet §2's dog condition would otherwise block on the §4 encounter table, which is two landings away.
Tried/failed: THREE false-positive gate tests in a row before the gate was trusted. (a) `Get-Content X | Set-Content X` is a same-file pipeline and silently did nothing; (b) truncation worked but a stale .git/index.lock — left by this agent's own earlier `git add -A` — was failing every `git add` with exit 128, and the resulting "nothing staged" was misread as "the mount caught up"; (c) never checked $LASTEXITCODE. Only attempt #4 genuinely fired. Same family as the synthetic-touchmove false positive: a test that cannot fail is not a test.
RETRACTED: SPEC-refactor-modularize.md §PREREQUISITE-GIT was false when written and ratified anyway — claimed the repo was abandoned at a2517ab with v14-v19 untracked and "six versions of work existing as loose files with no history." Verified on host: a2517ab is 77 commits back, all six TRACKED, tree clean. Nothing was ever lost. Origin: an agent read a stale sandbox mount, saw a 10h-old git log, built a crisis on it, wrote it in as a hard blocking prerequisite. Kept on the page as a retraction rather than deleted — this SPEC governs the autonomous-loop experiment and this IS that experiment's exposure. Not an agent that breaks the game (gates catch that). An agent that hallucinates a problem, solves it convincingly, and gets the fabrication ratified into the corpus. It cost nothing only because the phantom fix was a no-op on a clean tree.
Learned: the same stale mount produced BOTH symptoms — 508 phantom deletions (gateable, now gated) and a fabricated prerequisite in prose (NOT gateable, ever). One root cause, one symptom gets a gate, the other only ever gets a habit: verify state, don't narrate it. One `git ls-files` would have killed it before it was written. It wasn't run because the story was coherent, and coherent is not checked.
Learned: `Get-Content X | Measure-Object -Line` does NOT count blank lines and undercounts every file. Use `(Get-Content X).Count`. The gates use git/node and are unaffected.
Also fixed: README claimed "four gates" while the runner streams seven (table added). uploads/README.md still said "Not for distribution" inside a public MIT repo — two conflicting licenses in one tree; marked superseded, licensing defers to root LICENSE.
Gate: 7/7 PASS at every commit today (ccdbf75, 7df73f8, bb7dd23, + the Landing 3 spec). corpus-gate adversarially verified: VIBE.md gutted 345->200 and staged, gate failed exit 1 on both checks; override passes loudly and on the record; clean tree passes.
Next: Fable builds Landing 3 (concessions) on v20-concessions; chat Claude reviews + merges. Hook audit says it is two reads (night, dog) and two NEW clocks (choir hours, dryer cycle) — neither exists in source. Then Landing 4, world-relationship gate. Operator still owes: OD-6 (the Gone Rock) taste call, and the live in-browser feel pass for toast/feed/Q — a deterministic gate is still not a playtest. Game is live at sovereignprime357.github.io/rock-bottom (index 200, src/main.js 200 — modules genuinely serve).

## 2026-07-16 — SMOKE CONCESSIONS (landing 3) build (Fable / Claude Code, branch v20-concessions)

What: implemented SPEC-V20-PACKET §2 per SPEC-v20-concessions.md. Six commits on v20-concessions off main (da6f15a): (1) extraction — smokeRockAt() in new src/systems/concessions.js is the one rockedT=18000 site in src/, blockMenu calls it; (2) the two authored clocks — choir office hours (closed 90-150s / open 55s, the choir's schedule, not the sun's) and the dryer cycle (idle 50-90s / run 70s, mid-cycle = run minus 15s margins), ticked in updateWorld, one additive save key (concessionClocks), version 10 unchanged; (3) the rooms — conceded venues answer E as a lowest-priority zone verb, one smoke option + leave, condition stated never explained, 'consult the …' action hints, Q ledger concession line at conceded tier only; (4) BAD IDEA loop objective — rocks>0 ∧ rockedT<=0 ∧ shakes>=50 targets the nearest LEGAL spot, ties to the Block, placed under intro/contracts/kingdom and above office/routes/quests; (5) tools/concession-gate.mjs wired before runtime-smoke, suite 8/8; (6) corpus — SPEC.md landing section, REFACTOR-FINDINGS findings + OD-8, DELEGATION Landing 3 checked.

Why: the hook audit was the real shape of the job and it was correct — night and dog were reads; choir hours and dryer mid-cycle existed nowhere and are the build. Verified in source before building (grep: b flat is flavor at 5 sites, dryer is a sprite + palette + one line).

Decided (OD-8, veto standing): choir hours are an independent clock, NOT a dayTime window — the flavor defines hours by pitch; a window over the existing sun would be a read, not one of the two owed clocks; and it would make park∧choir unreachable by construction, turning 4 of the gate's 16 combination rows into theater. Also: any exact nearest-tie → Block (both tie kinds tested); conditions re-check at ACTION (stated in code; the world currently pauses during dialogue so the race is unreachable — the guard is for the engine where it isn't); BAD IDEA threshold shakes>=50 (half runway, ~60s of pointing before cap).

Tried/failed: first clock wiring used Math.random for phase durations — runtime smoke failed on NPC dialogue parity because the modular build runs in lockstep with frozen v19 on ONE shared seeded sequence and the clocks ate from it. Replaced with a private persisted LCG in the clock state; parity restored. Second: the identity fixture initially compared absolute before/after fingerprints — lifetime.rocksSmoked and faction accumulate across the fixture, and a spiritual tier crossing at -10 fired its own feed post that displaced the venue line under test. Fixed the fixture baseline, not the game.

Gate: concession-gate proves single-site (structural), 5-spot identity + soap parity via full-P fingerprint diff (reused from recognition-gate as ordered), tier/condition/stale-menu-race gating, Block-only coronation, 16-combo honest BAD IDEA with both tie shapes, mid-high expiry (dryer dies at t=1ms, high runs 18000 full, crash 8000 full), clock persistence + normalization + mid-cycle margin boundaries. RED-VERIFIED before trusted: coronation guard stripped → 4 failures exit 1; planted second rockedT=18000 → single-site failure exit 1. Both restored, 8/8 green.

Next: Landing 4 (world-relationship gate — runway/coverage budgets now have 5 potential legal spots to measure). Operator owes: review/merge of this branch; the live in-browser feel pass (deterministic gates are still not a playtest — the four rooms' menu copy, the clock toasts, and the Q lines have never been seen on a screen); the OD-8 veto window. Noticed: DELEGATION.md Landing 2 checkbox is still unchecked despite the ccdbf75 merge — left alone, operator's ledger to correct.

## 2026-07-16 — WORLD RELATIONSHIPS (landing 4) build (Fable / Claude Code, branch v20-world, own worktree)

What: implemented SPEC-V20-PACKET §3 per SPEC-v20-world-relationships.md. One new permanent gate (tools/world-gate.mjs, wired LAST in run-gates.mjs, suite: 9) and zero gameplay changes — git diff on src/ is empty at this landing. Built in a dedicated git worktree (v20-world) per ORCHESTRATOR-NOTES entry 7; never shared the orchestrator's checkout.

Why: the drift audit's finding — VIBE governs the sentence and is silent on scope. Every existing gate checks whether the game is correct; none checked whether it is still walkable. "Make it bigger" needed a mechanical opponent before the autonomous loop gets pointed at this game.

Premise check (ordered by the operator, done before building): all four numbers re-verified from source — baseSpeed 2.2 px/16ms (runtime_ui.js:33, applied as vx*spd*(dt/16) at update.js:82), withdrawal +0.0008/ms (update.js:124), fresh 20 → clamp 100 (runtime_ui.js:355, update.js:126), WORLD 8600×5600 (world.js:15). Runway 100s = 13,750px; diagonal 10,262.6px = 74.6s. No false premise found.

Decided (OD-9, veto standing): (1) "mandatory" includes rolled route legs over the full potentially-unlocked stop pool — the clipboard assigns, the player does not choose; excluding the endless pool would blind the gate to exactly the route-stop creep an autonomous loop will produce, and a gate row that can never fire is decoration (same reasoning the operator ratified in OD-8). (2) Coverage budget expressed as 45% of measured runway (= SPEC's ~45s at shipped values) so the ruler scales with the constants instead of rotting. (3) Gate runs LAST: it is expected to read red until the operator rules, and a standing world reading must never mask a regression in the seven behavior gates — the runner stops at first failure. (4) Budgets measured live each run: speed by actually walking the player and cross-checking the model, withdrawal by standing still, cap against the clamp; leg inventory read through the real objective selectors (currentPrimaryObjective / currentOfficeObjective / currentKingdomObjective) by driving quest/stage state, and route pools sampled through the real roller — nothing transcribed that can silently drift. (5) The failing leg stands. Budget not widened, no grandfather list — a grandfathered baseline is deleting the instrument with extra steps.

The reading: suite is 8 PASS + one honest world-gate failure — route scrap_gate ↔ ditch_gauge, 8,274.6px (60.2s) vs 8,250px (60s), over by 0.3%, straight-line lower bound. It is the same arithmetic as the Wave-4 table's 16,398px worst two-leg. Near misses recorded: block→throne banner at 95.8% of budget (the mandatory emperor commute is one outward nudge from red), ditch↔pawn_sill 98.5%. Coverage green (worst point 34.0s from the laundromat, 75% of budget); day-1 floor green (farthest objective 25% of budget); one rideable cart.

Tried/failed: (a) first suite-exit check read `node ... | tail; echo $?` — that's tail's exit code, not the runner's; the suite "passed" while world-gate was failing inside the pipe. Re-ran without the pipe: exit 1 as designed. Same family as ORCHESTRATOR-NOTES #3/#7 — the check was true and answering the wrong question. (b) early-exit path originally called the full report() before the derived constants existed (TDZ crash if the budget couldn't be derived); split the pre-budget bail into its own plain failure print.

Red-verified before trusted (all restored green after): choir venue moved to x=2000 → COVERAGE fails naming point (8600,5600) and the budget; throne banner moved to x=8560 → RUNWAY-CAMPAIGN fails naming "block -> throne banner" 8,650.7px; second cart planted in PROPS → TRANSPORT fails counting 2. Plus the genuine route red verified by hand arithmetic (dx 6760, dy 4772 → √68,469,584 = 8,274.6).

Docs: SPEC.md landing 4 section; REFACTOR-FINDINGS Landing 4 findings + OD-9; DELEGATION Landing 4 checked. Corpus untouched otherwise.

Next: operator rules on the scrap_gate↔ditch_gauge finding (move a stop / split the leg / authored exception with register entry / let it stand red). Note for that ruling: the near-miss table says the emperor commute is already at 95.8% — any "make it bigger" wave will trip RUNWAY-CAMPAIGN next. Operator still owes the Landing 3 feel pass and the OD-8 veto window; this landing adds the OD-9 veto window. Branch v20-world pushed, not merged.

## 2026-07-16 — ROUTE BUDGET (landing 5) build (Fable / Claude Code, branch v20-world, same worktree)

What: resolved OD-9 per SPEC-v20-route-budget.md (main 6f57af8, read via git show — main was not merged into the branch). Constrained the generator: rollBlockRoute learned the runway leg budget; routeLegBudgetPx() derives it at run time; world-gate reworked to measure the GENERATOR rather than the raw table and to certify the src derivation against the measured world. Suite 9/9 honestly green, world-gate included. Diff scope: one edited function + one new pure helper in progression_routes.js, plus the gate. No stop moved; no other src file touched.

Why: the Landing 4 finding was a generator with no distance awareness handing out the one illegal pair its table contains — 252 of 253 pairs fit, the worst missing by 24px. Constraining the roller makes the invariant unrepresentable instead of detected, without moving the budget, the content, or the meaning of the instrument.

Decided: (1) Selection stays the same shuffle (identical Math.random consumption — frozen-v19 lockstep parity in runtime-smoke verified unaffected; the smoke also never rolls a route pre-intro and never snapshots blockRoute) with budget-aware take: first remaining candidate within budget of the previous stop, relax-to-nearest fallback, never null. Identical to first-three whenever first-three is legal. (2) I-NO-BUDGET-DRIFT implementation: the SPEC forbids a second hardcoded 8250 and demands run-time derivation; the model inputs (2.2/16, 0.0008, 20, 100) are inline literals owned by runtime_ui/update, so a src derivation must either hoist them (multi-file refactor, violates one-function scope) or restate them. Chose restate-and-gate: world-gate measures the same quantities behaviorally (walking the player, standing still, overfilling the clamp) and fails on any divergence — a second copy pinned to the first by a gate is a tripwire, not a lie. Stated for veto. (3) Generator sampling lives INSIDE world-gate, not a tenth gate, because both the SPEC and the operator pin the revert-red acceptance to world-gate itself. The SPEC's "10/10" acceptance line therefore looks like a miscount — the suite is 9 gates and Landing 5 adds none. Flagged to the operator rather than padding the suite to make a number match. (4) The gate's route reading changed from max-pairwise-over-table to sampled-generator-output (10,000 rolls per config, chained lastStopId, zero over-budget, zero nulls) + informational table line + satisfiability proof (min 21 within-budget partners). Landing 4's pairwise reading was the right instrument for an unconstrained generator; with a constrained one it would fail forever on pairs that can never be assigned — measuring the wrong thing on purpose is decoration too.

Tried/failed: nothing discarded this landing. Two near-traps dodged by checking first: the runtime smoke's lockstep parity (would have gone red if the roller consumed different randoms or the smoke snapshot included blockRoute — it does neither, verified by reading runtime-smoke.mjs before writing code), and the revert-red test crashing instead of failing cleanly (a full-file revert removes routeLegBudgetPx and TypeErrors the gate; reverted only the selection block so the red is the named, honest kind).

Red-verified before trusted: (a) constraint reverted to blind first-three -> RUNWAY-ROUTES-FULL-UNLOCK fails naming scrap_gate<->ditch_gauge, 69 assigned over-budget legs in 10,000 rolls — the fix is provably what makes the gate green; (b) routeLegBudgetPx forced to 10px -> every pick in 30,000 rolls falls to the relax-to-nearest fallback, all return three distinct valid stops, no hang, no null (I-ROLL-TOTAL holds under the pathological case) AND NO-BUDGET-DRIFT fires naming both numbers. Both restored; suite 9/9.

The reading after the fix: worst assignable route leg is pawn_sill -> ditch_gauge at 8,123.8px = 98.5% of budget (I-STILL-CRUEL — one pair removed, no mercy added); day-1 worst assigned leg 1,826.7px; table still contains its one over-budget pair, reported as generator-excluded.
...
## 2026-07-16 — THE ECONOMY GATE (v21, Fable / Claude Code, branch v21-economy)

What: built `tools/economy-gate.mjs` per SPEC-v21-economy-gate.md — the mechanical answer to "what else is broken" from the operator's playtest report. A static census of every resource-grant site in src/ (81 sites: cash, rocks, soap rocks, copper, supplies, dirty supplies, inventory pushes), each claimed exactly once as a TRADE (36), a register-cited LEDGER payout (44, citing F-ECON-PAYOUTS), or a KNOWN VIOLATION (1: F-SOCK). The judgment half is behavioral: the gate loads the modular game in the VM harness, instruments the lanes with accessor traps that stack-attribute every grant to its source line, and drives 179 menus / ~10,100 option runs through the real disabled flags under a BROKE profile and a STOCKED profile. Trades must never pay without a same-action consumption and never leave a lane negative; deterministic trades must be observed conserving or the gate fails on coverage. F-ECON-PAYOUTS register section added to REFACTOR-FINDINGS.md; suite wired at position 11 (last); README table row added. Zero changes to src/ — the sock is NOT fixed, per the standing order.

Why: F-SOCK proved the infinite-resource-loop class exists in shipped code and nothing was watching it. A grep finds assignment sites, not violations — it cannot tell a guarded sale from a free dollar — so the gate reasons about the guard and the consumption together, the recognition-gate approach (diff the state, don't trust the labels) pointed at money.

Decided: (1) The sock is a KNOWN VIOLATION, not a ledger entry — a bug signed into a payout register is a grandfather list (OD-9's family). The gate ships red naming it, with live evidence ("BROKE: paid +1 cash, nothing consumed, inventory empty"), until the operator makes the sock call. (2) economy-gate takes the LAST slot, inheriting world-gate's standing-red placement rationale; world-gate reads green since Landing 5 so nothing is masked. OD-9's letter said world-gate runs last — its spirit (standing reds sit last) is what survived; flagged for the operator, swap is one line. (3) Site identity is (file, normalized statement, occurrence) — the sock's grant line is byte-identical to the license plate's nine lines down, so line-content-only identity was never going to work, and line numbers rot. (4) Conservation means "something was staked in the same action," not "net <= 0" — the price guy's $200 return and the lottery are gambles, not mints. (5) Equipment/weapons/cred/brain/hp are out of census scope, stated in the gate header: no sell-back path exists, so they cannot mint. (6) giveBusPass is one grant site with a paid caller and a free caller; site granularity cannot split callers, so it is ledgered with the reason written down.

Found, recorded, not fixed: the census caught one payout nobody classified on its first run — the jogger's "ask for $1" (npc_spawns.js:129, 40%, uncapped). Judged in-family with marketplace panhandling and ledgered with a note. Not a second F-SOCK: it pays pennies for a verb that is supposed to pay for nothing. The operator's "couple money bugs" reading: the second half was probably F-GHOST (walk-through buildings), which is not an economy defect; across every menu the driver reached, the sock is the only site that pays without staking, guarding, or being licensed.

Tried/failed: (a) first classification pass keyed pete's three identical-suffix pay lines by substring — the copper line contains the sock line as a suffix, so includes-matching triple-claimed; fixed with exact-match mode plus occurrence index. (b) A consumption-strip red test that edits the grant statement itself goes red through the census (stale entry + unclassified site) rather than the behavioral path — statement-keyed identity means ANY edit to a classified money line demands reclassification, which is correct but wasn't the assertion under test; the license plate (consumption on a separate line) isolated the behavioral catch.

Red-verified before trusted, every direction: (1) out of the box — standing red names F-SOCK with live BROKE-profile evidence; (2) guard stripped from pete's copper sell — red, "copper=-1, a purchase its guard did not fund"; (3) consumption stripped from the license sale — red, "granted +20 cash with NOTHING consumed," plus the coverage failure; (4) ledger cite swapped to a phantom token — red, unsigned ledger; (5) stale entry (from 3's statement edit) — red, "matches no census site"; (6) planted P.cash += 5 in larry — red, unclassified census site; (7) economy-gate row deleted from README — docs-gate red, exactly as entry #9 promised. All reverted; final state re-verified.

Drive-by doc fix, named: README's runner sentence claimed "seven" gates while ten ran (entry #6's class, pre-existing) — reworded count-free so it cannot rot again; the gated table is the authority.

Standing state at push: suite = 10 PASS + economy-gate's one honest red naming the sock. Exit 1 is the correct reading, not a defect in the branch.

Next: operator reviews (orchestrator re-runs the red tests above), rules on the sock (findable item vs "you always have another sock"), and ratifies or amends F-ECON-PAYOUTS — its 44 rows were authored by an agent and are exactly the ledger-growth-needs-human-eyes case OD-10 names. Not merged here.
